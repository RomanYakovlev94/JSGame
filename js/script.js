...
/**
 * Данная функция из массива объектов рандомно выбирает билет и проверяет его на содержание в массиве правильных ответов.
 * Если билет содержется в правильных ответах - то функция не отрисовывает страницу, а сначала выбиррает другой билет.
 * Далее, при нажатии на ответ - срабатывает событие и идет проверка на соответствие правильному ответу.
 * Отрисовывается страница развернутого объяснение правильного ответа, меняется счетчик правильных ответов (Тогда билет попадает в массив правильных ответов) или неправильных.
 * На кнопку reload вешается слушатель при рендере страницы, тк изначально его повесить не представляется возможнымю
 */
let render = () => {
...
/**
 * Данная ф-ция, после нажатия на любой ответ, отрисовывает развернутое объяснение, сам вопрос. Прри отрисовке добавляется кнопка перезагрузки и вешается событие.
 */
...
/**
 * Первый запуск игрры на стартовой странице
 */
